...
} from '@common/models/vendor.model';
import { VendorService } from '@super-admin/services/vendor.service';
...
export class Vendors implements OnInit {
  ...
  onGridAction(event: DataGridActionEvent<VendorAccount>): void {
    ...
    switch (event.actionId) {
      ...
      case 'set-inactive':
        ...
        break;
      case 'reset-password':
        this.resetOwnerPassword(vendor);
        break;
    }
  }
  ...
  async changeVendorStatus(vendor: VendorAccount, uiStatus: 'active' | 'inactive'): Promise<void> {
    if (this.statusUpdatingId()) {
      return;
    }

    const makingActive = uiStatus === 'active';
    const confirmed = await this.confirmDialog.confirm({
      title: makingActive ? 'Set Vendor Active' : 'Set Vendor Inactive',
      message: makingActive
        ? `Set "${vendor.name}" as active? The vendor will be able to access the account again.`
        : `Set "${vendor.name}" as inactive? The vendor will lose access until reactivated.`,
      confirmLabel: makingActive ? 'Set Active' : 'Set Inactive',
      cancelLabel: 'Cancel',
      tone: makingActive ? 'default' : 'danger',
    });
  ...
  /** Confirm, then call API — password is generated by the server, then show credentials modal. */
  async resetOwnerPassword(vendor: VendorAccount): Promise<void> {
    ...
    const confirmed = await this.confirmDialog.confirm({
      ...
      message: `Reset owner password for "${vendor.name}"? A new password will be generated.`,
  ...
  submitVendorForm(): void {
    ...
    this.vendorForm.currency = this.vendorForm.currency || 'INR';
  ...
  private handleVendorSaved(vendor: VendorAccount): void {
    ...
    if (isNew) {
      ...
      this.toastService.success('Vendor added successfully.');
    } else {
      this.allVendors.set(
