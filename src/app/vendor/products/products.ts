...
export class VendorProducts implements OnInit {
  ...
  /** Public website cover (base64 / URL) — shown before OTP. */
  readonly coverImage = signal('');
  /** Extra product images — shown only after customer OTP verify. */
  ...
  getCatalogName(catalogId: number): string {
    return this.catalogs().find((c) => c.id === catalogId)?.name ?? '—';
  }
  ...
  private readFileAsBase64(file: File, onDone: (base64: string) => void): void {
    ...
    // ~1.5MB original ≈ ~2MB base64 — keep payloads manageable
